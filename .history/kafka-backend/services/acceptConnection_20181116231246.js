  ...
  Applicant.findById(msg.userId).then(profile=>{
      if(!profile)
      {
        callback(null,"User doesn't exist")
          ...
          newConnection.save().then(console.log("One side connection created")).catch(err=>callback(err,"Error creating connection"));
        ...
        newConnection1.save().then(console.log("two sided connection created")).catch(err=>callback(err,"Error creating connection"));
