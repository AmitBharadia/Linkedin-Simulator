...
async function handle_request(msg, callback) {
    ...
    // msg.query+=" ,{ $or : [ { price : 0.99 }, { price : 1.99 } ] }"

    // q["company"] = { $regex: req.query[key], $options: "i" }
    console.log("In handle request:" + JSON.stringify(msg));
    try {
        let jobs = await getAllJobs(msg.query);
        console.log("Reslult: ", JSON.stringify(jobs));
...
function getAllJobs(q) {
    return Jobs.find(q, {
        description: 1, _id: 1, job_id: 1, position: 1, location: 1,
        easyApply: 1,
        company: 1,
        company_logo: 1,
        employment_type: 1,
